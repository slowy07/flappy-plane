...
def create_building():
    ...
    top_building = building.get_rect(
        midbottom=(DISPLAY_WIDTH + 26, random_building - 150)
    )
...
def move_floor():
    ...
    display.blit(
        floor, (floor_pos_x + DISPLAY_WIDTH, DISPLAY_HEIGHT - floor_rect.height // 2)
    )
...
def check_collision(buildings):
    ...
    if (
        plane_rect.top <= 0
        or plane_rect.bottom >= DISPLAY_HEIGHT - floor_rect.height // 2
    ):
        collide_snd.play()
        return False
...
def score_display(game_state):
    if game_state == "main_game":
        ...
        high_score_font_rect = high_score_font.get_rect(
            center=(DISPLAY_WIDTH // 2, 412)
        )
